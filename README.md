...
# 3. 構築。UIを手元ブラウザで見るため 443 を開ける。
#    大学等の FW では手元の見かけ上のグローバルIP（NAT）と実際の送信元がズレて /32 だと
#    届かないことがあるため、本ラボは 0.0.0.0/0（全公開）で統一する。
#    ⚠️全公開・要注意（CALDERAはログインあり／Guacamoleは自動生成パスワードで保護）。
terraform init
terraform apply -auto-approve -var "ui_cidr=0.0.0.0/0"
#    ※既定値も 0.0.0.0/0 なので、-var を省いた `terraform apply -auto-approve` でも同じく全公開になる。
#    　特定IP/キャンパスCIDRに絞れる環境なら -var "ui_cidr=x.x.x.x/32" で限定してもよい。
...
| `ui_cidr` | `0.0.0.0/0` | UI(HTTPS/443)を開ける送信元。**既定は全公開**（大学FW越しでも確実に到達させるため統一）。`x.x.x.x/32` やキャンパスCIDRに限定も可。`""`=非公開(SSM運用) |
...
サーバ上の **Caddy** が `443/HTTPS → CALDERA(8888)` を中継しています。`ui_cidr`（既定 `0.0.0.0/0`）で
443 が開いているので、**手元ブラウザで直接アクセスできます**（aws CLI も session-manager-plugin も不要）。
大学FWが 443 を通す限り到達可能。

```bash
# CloudShell で（apply 済みなら、あとはURLを開くだけ）
terraform output -raw ui_url
...
- 本ラボは `ui_cidr=0.0.0.0/0`（全公開）で統一。自分のIPを調べて `/32` で絞る必要はない（大学FWの
  NAT で送信元がズレて届かないことがあるため）。絞れる環境で限定したい場合のみ https://ifconfig.me
  を**手元ブラウザで**開いて確認（CloudShellのcurlは不可）
...
```bash
# UIと同じ 443（ui_cidr、既定 0.0.0.0/0）で開いている。
...
cd caldera-aws-lab
terraform apply -auto-approve              # 変わった public IP を state に反映（再作成はされない）。ui_cidr は既定 0.0.0.0/0 なので -var 不要
...
- **エージェントが出てこない**: 通常は60分リトライ内に自動登録される。それでも出ない場合は
  `terraform apply -replace=aws_instance.victim[0]` でやられ役だけ作り直すと、起動済みサーバへ即コールバックします（`ui_cidr` は既定 `0.0.0.0/0` なので `-var` を付け直さなくても443ルールは維持される。個別に絞っている場合のみ同じ `-var "ui_cidr=..."` を付け直す）。
